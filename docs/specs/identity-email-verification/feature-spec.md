...
- [x] **AC-1:** A successful registration creates **exactly one** row in
...
- [x] **AC-2:** The plaintext token appears in **no** database column, **no** log line, **no**
...
- [x] **AC-3:** Exactly one email is sent per issued request, addressed to the user's own address,
...
- [x] **AC-4:** The email's `From` is the configured no-reply sender, and its body renders no address
      other than the recipient's own (Constitution §8).
- [x] **AC-5:** Registration still succeeds — 302, user row committed — when the mail transport
...
- [x] **AC-6:** The absolute URL is built from the configured base URI and is correct when the mail
...
- [x] **AC-7:** `GET /verify-email/{token}` with a valid, unexpired, unredeemed token sets
...
- [x] **AC-8 (single use — the important one):** Replaying the **same** URL after a successful
...
- [x] **AC-9:** A request that is already redeemed while its user is somehow **not** verified (a
...
- [x] **AC-10:** An **expired** token (clock advanced past `expires_at`) redirects to the resend form
...
- [x] **AC-11:** An **unknown** token (well-formed, never issued) produces a response
      **indistinguishable** from the expired case — same status, same `Location`, same flash string.
- [x] **AC-12:** A **malformed** token (wrong length, non-base64url characters, empty, 10 kB of junk)
...
- [x] **AC-13:** Redemption performs **no** password check and starts **no** session: the user is
...
- [x] **AC-14:** `GET /verify-email/resend` returns 200 with a single `email` field and a CSRF token.
- [x] **AC-15 (no user enumeration):** `POST /verify-email/resend` returns a **byte-identical**
...
- [x] **AC-16:** Case (a) creates a **new** request row with a **different** `token_hash` and sends
...
- [x] **AC-17 (domain rate limit):** The **6th** issuance for the same user inside a rolling hour is
...
- [x] **AC-18 (boundary rate limit):** The resend endpoint is additionally limited to **5 POSTs per
      ...
      filesystem.
- [x] **AC-19:** The resend flow never reveals a token, a request id, or any timestamp of a previous
      request.

### Enforcement at login (inverting slice 1's AC-24)

- [x] **AC-20 (the inversion):** A user with `email_verified_at IS NULL` posting **correct**
      ...
      discharged.
- [x] **AC-21 (enumeration-safe placement):** The same unverified user posting a **wrong** password
...
- [x] **AC-22:** A verified user's login is unaffected: 302 to `/account`, session established,
      slice-1 behaviour intact.
- [x] **AC-23:** The failed unverified login **counts** against `login_throttling` exactly like any
...
- [x] **AC-24 (supersedes slice 1 AC-3):** After a successful registration the redirect target is
...
- [x] **AC-25:** Every slice-1 functional test that authenticates is updated to build a **verified**
...
- [x] **AC-26:** Issuing a request dispatches exactly one `EmailVerificationRequested` carrying the
      request id, the user id, `issuedAt` and `expiresAt` — and **no token and no email address**.
- [x] **AC-27:** A successful redemption dispatches exactly one `UserEmailVerified` (slice 1's event,
      unchanged). A replayed redemption (AC-8) dispatches **none**.
- [x] **AC-28:** A registration whose verification mail fails to send still dispatches
      `UserRegistered`; a mail failure neither rolls back nor suppresses the fact.

### The console command

- [x] **AC-29:** `muzbar:identity:verify-email <email>` still works, unchanged, and still exits 0 on
...
- [x] **AC-30:** The token is stored only as a digest; a dump of
      `identity_email_verification_request` contains nothing that can be replayed into a URL.
- [x] **AC-31:** Token comparison inside the aggregate uses `hash_equals` (constant time), asserted
      in review and by a unit test proving a one-character-off token is rejected.
- [x] **AC-32:** No response, page, flash or email in this slice renders an address other than the
      one the visitor supplied or the recipient's own (Constitution §8).
- [x] **AC-39:** The `GET /verify-email/{token}` response carries `Referrer-Policy: no-referrer`. The
...
- [x] **AC-33 (Domain purity):** `grep -rE '^use (Symfony|Doctrine)\\' src/src/Domain/` still returns
...
- [x] **AC-34:** `EmailVerificationRequest` holds a `UserId`, **not** a `User`: no Doctrine
...
- [x] **AC-35:** The new table follows ADR-0007 exactly — name
...
- [x] **AC-36:** The migration is additive and reversible: `migrate prev` drops the new table
      cleanly, leaves `identity_user` untouched, and re-migrating succeeds.
- [x] **AC-37 (index, not a latency budget):** `EXPLAIN` of the redemption lookup
...
- [x] **AC-38:** `make check` is green: php-cs-fixer clean, PHPStan **max** zero errors, Deptrac zero
