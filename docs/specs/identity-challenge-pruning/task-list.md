...
- [x] **T2:** `Entity/EmailVerificationRequest` — `RETENTION_AFTER_EXPIRY_SECONDS = 604800` and
      ...
      **(AC-1, AC-2, AC-30)**
- [x] **T3:** `Entity/PasswordResetRequest` — `RETENTION_AFTER_EXPIRY_SECONDS = 2592000` and
...
- [x] **T4:** `Port/EmailVerificationRequestRepository` and `Port/PasswordResetRequestRepository` —
...
- [x] **T5:** Checkpoint — `make stan deptrac`; confirm zero framework imports under `Domain/`;
...
- [x] **T6:** `ChallengePruningReport` + `SweepOutcome` — `final readonly`, directly under
      ...
      different moment.
- [x] **T7:** `Command/PruneExpiredChallenges` (`?int $limit`, `bool $dryRun`) +
...
- [x] **T8:** `DoctrineEmailVerificationRequestRepository` + `DoctrinePasswordResetRequestRepository`
...
- [x] **T9:** `Persistence/Doctrine/ChallengeIntegrityProbe` — DBAL `NOT EXISTS` anti-join per table,
      ...
      *"does this row's user still exist?"* is a storage-integrity question, not a domain one; no use
      ...
      later. **(AC-25)**
- [x] **T10:** Migration — two `CREATE INDEX` statements on `expires_at`, plus the matching
...
- [x] **T11:** `config/packages/monolog.yaml` — add the `pruning` channel. One line, its own commit,
      because everything downstream logs to it.
- [x] **T12:** `Console/PruneChallengesCommand` (`muzbar:identity:prune-challenges`) — `--dry-run`,
...
- [x] **T13:** `HealthController::ready()` — add the **reporting-only** `jobs.challenge_pruning`
...
- [x] **T14:** Test support — a `ClearsPruningState` trait for the two new Redis keys (or extend
...
- [x] **T15:** Domain unit — `retentionThreshold()` on both aggregates: exact instants, UTC and
...
- [x] **T16:** Integration — both adapters' new methods: **both sides of the boundary**
...
- [x] **T17:** Integration — the two rows that can only be made with raw SQL, each with a docblock
...
- [x] **T18:** Integration — `ChallengeIntegrityProbe`: a hand-inserted orphan is counted **and is
      still present afterwards**; an **overdue** orphan is removed by the ordinary sweep with no
      special handling. **(AC-25, AC-26)**
- [x] **T19:** Integration — `PruneExpiredChallengesHandler` with `FrozenClock`: one `now()` pins
...
- [x] **T20:** Functional — the command via `CommandTester`: exit 0 on a clean run; exit 0 and
...
- [x] **T21:** Functional — `/health/ready`: the new section's shape; **200 with an absent
...
- [x] **T22:** Functional — **AC-24's distinguishability test**, as *one* readable test with two
...
- [x] **T23:** Functional — **AC-6 end-to-end**: issue a reset, redeem it, run a sweep, replay the
...
- [x] **T24:** Infrastructure assertions — `EXPLAIN` shows an Index Scan on each new index for the
...
- [x] **T25:** Ops docs — `docs/infrastructure.md`: the **runbook entry** (the exact cron line
...
- [x] **T26:** Docs: `CLAUDE.md` (the first `DELETE` in the codebase and the rule that licences it —
